...
protected:
	...
	UPROPERTY(BlueprintReadOnly, EditDefaultsOnly, Category = "ACF | Damages")
	TMap<FGameplayTag, FEffectByDamageType> DamageEffectsByHitReaction;
...
public:
	...
	UFUNCTION(BlueprintCallable, Category = ACF)
	void PlayHitReactionEffect(const FGameplayTag& HitRection, const TSubclassOf<class UDamageType>& DamageType, ESpawnFXLocation locationSpawn = ESpawnFXLocation::ELastDamageLocation,  const FName& boneOrSocketName = NAME_None);
...
private: 
	...
	void PlayCurrentActionEffect(FEffect outEffect, ESpawnFXLocation locationSpawn, const FName& boneOrSocketName);
